import adv1 from '../../shared/assets/adv_list/adv_1.svg'
import adv2 from '../../shared/assets/adv_list/adv_2.svg'
import adv3 from '../../shared/assets/adv_list/generationView.png'
import adv4 from '../../shared/assets/adv_list/adv_4.png'


export const advantagesData = [
    {
        id: adv1,
        img: adv1,
        label: "Генерировать карточки с помощью искусственного интеллекта",
    },
    {
        id: adv2,
        img: adv2,
        label: "Достигать целей вместе с персонажем"
    },
    {
        id: adv3,
        img: adv3,
        label: "Использовать умный тренажер для изучения",
    },
    {
        id: adv4,
        img: adv4,
        label: "Добавлять свои слова в приложение"
    },
];